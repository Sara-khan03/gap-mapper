...
def analyze_resume(file):
    ...
    elif file.name.endswith(".docx") and docx2txt:
        text += docx2txt.process(file)
    elif file.name.endswith(".txt"):
        text += str(file.read(), "utf-8")
    else:
        return "⚠️ Unsupported file format or missing library."
    
    if not text.strip():
        return "⚠️ Couldn’t extract text from resume."
...
if page == "Home & Resume Analyzer":
    ...
    try:
        response = requests.get("https://devpost.com/api/hackathons?status=open", timeout=5)
        data = response.json()
